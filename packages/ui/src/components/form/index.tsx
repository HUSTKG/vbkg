...
export type FieldConfig = {
	...
	'radio' | 'switch' | 'date' | 'hidden' | 'object' | 'array';
	placeholder?: string;
	description?: string;
	defaultValue?: any;
	required?: boolean;
...
const GeneralForm = <T extends z.ZodType>({
	...
	const initialFields = typeof fieldsRef.current === 'function'
		? fieldsRef.current({})
		: fieldsRef.current;

	// State for dynamic fields
	const [currentFields, setCurrentFields] = useState<FieldConfig[]>(initialFields);

	// Flag to prevent infinite updates
	const isUpdatingFields = useRef(false);

	// Prepare default values for the form
	const formDefaultValues = initialFields.reduce((acc, field) => {
		if (defaultValues[field.name] !== undefined) {
			...
			switch (field.type) {
				...
				case 'object':
					if (field.fields) {
						acc[field.name] = field.fields.reduce((objAcc: Record<string, any>, nestedField) => {
							if (nestedField.defaultValue !== undefined) {
								...
								switch (nestedField.type) {
									case 'checkbox': objAcc[nestedField.name] = []; break;
									case 'switch': objAcc[nestedField.name] = false; break;
									case 'date': objAcc[nestedField.name] = null; break;
									case 'object': objAcc[nestedField.name] = {}; break;
									case 'array': objAcc[nestedField.name] = []; break;
									default: objAcc[nestedField.name] = '';
								}
							}
							return objAcc;
						}, {});
					...
					break;
				case 'array':
					acc[field.name] = field.minItems && field.minItems > 0 && field.fields
						? Array.from({ length: field.minItems }).map(() =>
							field.fields?.reduce((itemAcc: Record<string, any>, itemField) => {
								if (itemField.defaultValue !== undefined) {
									itemAcc[itemField.name] = itemField.defaultValue;
								} else {
									// Set default based on item field type
									switch (itemField.type) {
										case 'checkbox': itemAcc[itemField.name] = []; break;
										case 'switch': itemAcc[itemField.name] = false; break;
										case 'date': itemAcc[itemField.name] = null; break;
										case 'object': itemAcc[itemField.name] = {}; break;
										case 'array': itemAcc[itemField.name] = []; break;
										default: itemAcc[itemField.name] = '';
									}
								}
								return itemAcc;
							}, {})
	...
	const renderNestedField = (field: FieldConfig, path: string, disabled: boolean = false) => {
							...
							<div className="flex items-center justify-between">
								...
								{!field.disabled && !isLoading && (
									<Button
										...
										size="sm"
										onClick={() => {
											const newItem = field.fields?.reduce((acc: Record<string, any>, itemField) => {
												if (itemField.defaultValue !== undefined) {
													acc[itemField.name] = itemField.defaultValue;
												} else {
													switch (itemField.type) {
														case 'checkbox': acc[itemField.name] = []; break;
														case 'switch': acc[itemField.name] = false; break;
														case 'date': acc[itemField.name] = null; break;
														case 'object': acc[itemField.name] = {}; break;
														case 'array': acc[itemField.name] = []; break;
														default: acc[itemField.name] = '';
													}
												}
												return acc;
											}, {});
											...
											if (field.maxItems === undefined || fieldArray.fields.length < field.maxItems) {
												fieldArray.append(newItem);
											}
										}}
										disabled={(field.maxItems !== undefined && fieldArray.fields.length >= field.maxItems) || field.disabled || isLoading}
									>
										...
										<Card key={item.id}>
												...
												<>
													<CardHeader>
														<CardTitle className="text-base flex items-center justify-between">
															<span>{field.arrayItemLabel || 'Item'} {index + 1}</span>
														</CardTitle>
													</CardHeader>
													<CardContent className="space-y-4">
														{field.fields?.map((nestedField) => (
											...
											<CardFooter className="flex justify-between border-t px-6 py-3">
												<div className="flex space-x-2">
													...
													{index < fieldArray.fields.length - 1 && !field.disabled && !isLoading && (
														<Button
															type="button"
															variant="outline"
															size="sm"
												...
												{!field.disabled && !isLoading && (
													<Button
														...
														onClick={() => {
															...
															if (field.minItems === undefined || fieldArray.fields.length > field.minItems) {
																fieldArray.remove(index);
															}
														}}
														disabled={(field.minItems !== undefined && fieldArray.fields.length <= field.minItems) || field.disabled || isLoading}
													>
